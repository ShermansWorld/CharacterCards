...
public class KonquestHook {
	...
	public static void displayKonquestInfo(Player player, String playerName) {
		try {
			player.sendMessage(Helper.color("&3Kingdom&8 - &b" + Konquest.getInstance().getPlayerManager()
					.getOfflinePlayer(Bukkit.getOfflinePlayer(Bukkit.getPlayer(playerName).getUniqueId())).getKingdom()
					.getName()));
		} catch (NullPointerException e) {
		}
	}
	...
	public static void displayKonquestInfo(Player player, Player target) {
		try {
			player.sendMessage(Helper.color("&3Kingdom&8 - &b" + Konquest.getInstance().getPlayerManager()
					.getOfflinePlayer(Bukkit.getOfflinePlayer(target.getUniqueId())).getKingdom().getName()));
